 ...
 <c:if test="${not empty cmessage}">
        ...
        <div style="float:right">
                        ...
                        <c:set var="tobepicked" value="0"/>
                                        <c:set var="tobedelivered" value="0"/>
       ...
       <%--  <c:set var="cou" value="0"/>
         ...
         <c:if test="${orderDetailInfo.productStatus=='Order-Confirmed'}">
                  <c:set var="tobepicked" value="${tobepicked+1}"/>
         </c:if>
          <c:if test="${orderDetailInfo.productStatus=='Pickedup from Seller'}">
                  <c:set var="tobedelivered" value="${tobedelivered+1}"/>
         </c:if>
          <c:if test="${orderDetailInfo.productStatus=='Delivered to Buyer'}">
                  <c:set var="delivered" value="${delivered+1}"/>
         </c:if>
         <c:set var="cou" value="${cou+1}"/>
         </c:forEach> --%>
         <%--  <c:if test="${cou==tobepicked}">
                <a href="${pageContext.request.contextPath}/deliveryboy/updateorderstatus?orderstatus=3&orderId=${orderInfo.id}&pids=${pids}" class="btn btn-primary btn-sm">Mark as Picked</a>
        </c:if>
        <c:if test="${cou==tobedelivered}">
                
                        <a href="${pageContext.request.contextPath}/deliveryboy/updateorderstatus?orderstatus=5&orderId=${orderInfo.id}&pids=${pids}" class="btn btn-primary btn-sm">Mark as Delivered</a>
        </c:if> --%>
...
<td>
<c:if test="${orderDetailInfo.productStatus=='Order-Confirmed'}">
                <a href="${pageContext.request.contextPath}/deliveryboy/updateorderstatus?orderstatus=3&orderId=${orderInfo.id}&pids=${orderDetailInfo.productId}" class="btn btn-primary btn-sm">Mark as Picked</a>
        </c:if>
        <c:if test="${orderDetailInfo.productStatus=='Pickedup from Seller'}">
                
                        <a href="${pageContext.request.contextPath}/deliveryboy/updateorderstatus?orderstatus=5&orderId=${orderInfo.id}&pids=${orderDetailInfo.productId}" class="btn btn-primary btn-sm">Mark as Delivered</a>
        </c:if>
